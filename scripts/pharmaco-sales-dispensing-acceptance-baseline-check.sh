...
section "Phase 21.0 PharmaCo360 Sales/Dispensing Acceptance Baseline Check"
...
section "3. Phase 20.9 evidence exists and passed"

PHASE_20_9="docs/pharmaco/sales-dispensing-post-ux-preview-evidence/PHASE_20_9_PHARMACO360_SALES_DISPENSING_POST_UX_PREVIEW_EVIDENCE_EVIDENCE_OUTPUT.txt"

if [ -f "$PHASE_20_9" ] && grep -q "PHASE 20.9 PHARMACO360 SALES/DISPENSING POST-UX PREVIEW EVIDENCE CHECK PASSED" "$PHASE_20_9"; then
  pass "Phase 20.9 evidence exists and passed."
else
  fail "Phase 20.9 evidence missing or failed."
fi

section "4. Phase 21.0 files exist"

DOC_FILE="docs/pharmaco/sales-dispensing-acceptance-baseline/PHASE_21_0_PHARMACO360_SALES_DISPENSING_ACCEPTANCE_BASELINE.md"
STATUS_FILE="docs/pharmaco/sales-dispensing-acceptance-baseline/PHASE_21_0_PHARMACO360_SALES_DISPENSING_ACCEPTANCE_BASELINE_STATUS.env"

if [ -f "$DOC_FILE" ]; then
  pass "Phase 21.0 acceptance baseline document exists."
else
  fail "Missing Phase 21.0 acceptance baseline document."
fi

if [ -f "$STATUS_FILE" ]; then
  pass "Phase 21.0 acceptance baseline status file exists."
else
  fail "Missing Phase 21.0 acceptance baseline status file."
fi

section "5. Phase 21.0 status values are correct"

if [ -f "$STATUS_FILE" ]; then
  if [ "$(read_status_value "$STATUS_FILE" "ACCEPTANCE_BASELINE_STATUS")" = "PREPARED" ]; then
    pass "Acceptance baseline status is PREPARED."
  else
    fail "Acceptance baseline status is not PREPARED."
  fi
  ...
  if [ "$(read_status_value "$STATUS_FILE" "SOURCE_PREVIEW_PHASE")" = "PHASE_20_9_POST_UX_PREVIEW_EVIDENCE" ]; then
    pass "Source preview phase is Phase 20.9."
  else
    fail "Source preview phase is incorrect."
  fi

  if [ "$(read_status_value "$STATUS_FILE" "PHASE_20_8_IMPLEMENTATION_MERGED")" = "YES" ]; then
    pass "Phase 20.8 implementation merge is YES."
  else
    fail "Phase 20.8 implementation merge is not YES."
  fi

  if [ "$(read_status_value "$STATUS_FILE" "PHASE_20_9_PREVIEW_EVIDENCE_MERGED")" = "YES" ]; then
    pass "Phase 20.9 preview evidence merge is YES."
  else
    fail "Phase 20.9 preview evidence merge is not YES."
  fi
  ...
  if [ "$(read_status_value "$STATUS_FILE" "ACCEPTANCE_STATUS")" = "READY_FOR_OWNER_REVIEW" ]; then
    pass "Acceptance status is READY_FOR_OWNER_REVIEW."
  else
    fail "Acceptance status is not READY_FOR_OWNER_REVIEW."
  fi
fi

section "6. No new implementation is included in Phase 21.0"
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "95" ]; then
    pass "Overall system completion is recorded as 95%."
  else
    fail "Overall system completion is not 95%. Current value: ${OVERALL:-MISSING}"
  fi
...
section "9. Acceptance baseline includes source phases"

if [ -f "$DOC_FILE" ]; then
  if grep -q "Phase 20.4" "$DOC_FILE" \
    && grep -q "Hotfix PR #104" "$DOC_FILE" \
    && grep -q "Phase 20.5" "$DOC_FILE" \
    && grep -q "Phase 20.6" "$DOC_FILE" \
    && grep -q "Phase 20.7" "$DOC_FILE" \
    && grep -q "Phase 20.8" "$DOC_FILE" \
    && grep -q "Phase 20.9" "$DOC_FILE"; then
    pass "Acceptance baseline includes required source phases."
  else
    fail "Acceptance baseline source phases are incomplete."
  fi
fi

section "10. Accepted working baseline is documented"

if [ -f "$DOC_FILE" ]; then
  if grep -q "Login and Home Landing" "$DOC_FILE" \
    && grep -q "PharmaCo360 Command Center" "$DOC_FILE" \
    && grep -q "Sales/Dispensing Queues" "$DOC_FILE" \
    && grep -q "Sales/Dispensing Filters" "$DOC_FILE" \
    && grep -q "Selected-Sale Review" "$DOC_FILE" \
    && grep -q "Payment and Prescription Workflow" "$DOC_FILE" \
    && grep -q "Responsive Review Baseline" "$DOC_FILE"; then
    pass "Accepted working baseline sections are documented."
  else
    fail "Accepted working baseline sections are incomplete."
  fi
fi

section "11. Acceptance criteria are documented"

if [ -f "$DOC_FILE" ]; then
  if grep -q "Admin dashboard build passes" "$DOC_FILE" \
    && grep -q "Backend tests pass in GitHub quality gate" "$DOC_FILE" \
    && grep -q "Public website build passes in GitHub quality gate" "$DOC_FILE" \
    && grep -q "Login lands on the dashboard home page" "$DOC_FILE" \
    && grep -q "Sales/dispensing dashboard renders without runtime crash" "$DOC_FILE" \
    && grep -q "No package archive or checksum is generated" "$DOC_FILE"; then
...
section "12. Owner review notes are documented"

if [ -f "$DOC_FILE" ]; then
  if grep -q "sales queue card clarity" "$DOC_FILE" \
    && grep -q "filter usability" "$DOC_FILE" \
    && grep -q "selected-sale detail readability" "$DOC_FILE" \
    && grep -q "payment guidance clarity" "$DOC_FILE" \
    && grep -q "prescription guidance clarity" "$DOC_FILE" \
    && grep -q "mobile/tablet responsiveness" "$DOC_FILE"; then
    pass "Owner review notes are documented."
  else
    fail "Owner review notes are incomplete."
  fi
fi

section "13. Responsive review baseline is present"
...
if [ -f "$DOC_FILE" ]; then
  if grep -q "360px small mobile" "$DOC_FILE" \
    ...
    pass "Responsive review baseline is present."
  else
    fail "Responsive review baseline is incomplete."
  fi
fi
...
if [ -f "$DOC_FILE" ]; then
  if grep -q "No production action is approved by this phase" "$DOC_FILE" \
    ...
    && grep -q "Final package generation authorization remains 0%" "$DOC_FILE" \
...
section "15. Only allowed Phase 21.0 files are changed"
...
DISALLOWED_FILES="$(
  echo "$CHANGED_FILES" | grep -Ev \
    '^(docs/pharmaco/sales-dispensing-acceptance-baseline/PHASE_21_0_PHARMACO360_SALES_DISPENSING_ACCEPTANCE_BASELINE\.md|docs/pharmaco/sales-dispensing-acceptance-baseline/PHASE_21_0_PHARMACO360_SALES_DISPENSING_ACCEPTANCE_BASELINE_STATUS\.env|docs/pharmaco/sales-dispensing-acceptance-baseline/PHASE_21_0_PHARMACO360_SALES_DISPENSING_ACCEPTANCE_BASELINE_EVIDENCE_OUTPUT\.txt|scripts/pharmaco-sales-dispensing-acceptance-baseline-check\.sh)$' || true
)"

if [ -n "$DISALLOWED_FILES" ]; then
  echo "$DISALLOWED_FILES"
  fail "Files outside the allowed Phase 21.0 acceptance/evidence scope are changed."
else
  pass "Only allowed Phase 21.0 acceptance/evidence files are changed."
fi
...
echo "Overall system completion: approximately 95%"
echo "Phase 21.0 PharmaCo360 sales/dispensing acceptance baseline check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 21.0 PHARMACO360 SALES/DISPENSING ACCEPTANCE BASELINE CHECK PASSED"
  exit 0
fi

echo "PHASE 21.0 PHARMACO360 SALES/DISPENSING ACCEPTANCE BASELINE CHECK FAILED with $FAILURES failure(s)."
exit 1
