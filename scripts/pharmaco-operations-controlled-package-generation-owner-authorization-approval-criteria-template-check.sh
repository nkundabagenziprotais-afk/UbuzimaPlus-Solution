...
section "Phase 19.2 Owner Authorization Approval Criteria Template Check"
...
section "4. Phase 19.2 approval criteria template exists"

DOC_FILE="docs/pharmaco/controlled-package-generation/owner-authorization-approval-criteria-template/PHASE_19_2_OWNER_AUTHORIZATION_APPROVAL_CRITERIA_TEMPLATE.md"
STATUS_FILE="docs/pharmaco/controlled-package-generation/owner-authorization-approval-criteria-template/PHASE_19_2_OWNER_AUTHORIZATION_APPROVAL_CRITERIA_TEMPLATE_STATUS.env"

if [ -f "$DOC_FILE" ]; then
  pass "Phase 19.2 approval criteria template document exists."
else
  fail "Missing Phase 19.2 approval criteria template document."
fi

if [ -f "$STATUS_FILE" ]; then
  pass "Phase 19.2 approval criteria status file exists."
else
  fail "Missing Phase 19.2 approval criteria status file."
fi

section "5. Approval template is prepared but not granted"

if [ -f "$STATUS_FILE" ]; then
  TEMPLATE_STATUS="$(read_status_value "$STATUS_FILE" "OWNER_AUTHORIZATION_APPROVAL_TEMPLATE_STATUS")"
  ...
  if [ "$TEMPLATE_STATUS" = "PREPARED_NOT_GRANTED" ]; then
    pass "Approval template status is PREPARED_NOT_GRANTED."
  else
    fail "Approval template status is not PREPARED_NOT_GRANTED. Current value: ${TEMPLATE_STATUS:-MISSING}"
  fi
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "88" ]; then
    pass "Overall system completion is recorded as 88%."
  else
    fail "Overall system completion is not 88%. Current value: ${OVERALL:-MISSING}"
  fi
...
section "8. Required owner approval wording is present"

if [ -f "$DOC_FILE" ]; then
  if grep -q "I, Protais Nkundabagenzi, explicitly authorize controlled local package generation" "$DOC_FILE" \
    && grep -q "does not approve production deployment, production migration, or production dependency installation" "$DOC_FILE"; then
    pass "Required owner approval wording is present."
  else
    fail "Required owner approval wording is missing or incomplete."
  fi
fi
...
echo "Overall system completion: approximately 88%"
echo "Phase 19.2 owner authorization approval criteria template check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 19.2 OWNER AUTHORIZATION APPROVAL CRITERIA TEMPLATE CHECK PASSED"
  exit 0
fi

echo "PHASE 19.2 OWNER AUTHORIZATION APPROVAL CRITERIA TEMPLATE CHECK FAILED with $FAILURES failure(s)."
exit 1
