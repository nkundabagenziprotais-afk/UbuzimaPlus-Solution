...
for file in "$HOLD_LEDGER" "$HOLD_EXCEPTION_LOG" "$PREFLIGHT_LEDGER" "$PREFLIGHT_EXCEPTION_LOG" "$RELEASE_LEDGER" "$RELEASE_EXCEPTION_LOG" "$READINESS_LOCK" "$READINESS_UNLOCK_LOG" "$CLOSURE_LEDGER" "$CHECKSUM_LEDGER" "$MANIFEST_LEDGER" "$EXCLUSION_LEDGER" "$EVIDENCE_MANIFEST" "$BINDER" "$AUTH_GATE" "$QA" "$ARCH"; do
  require_file "$file"
done

require_text "$QA" "Phase 16.9 controlled package generation command release hold ledger"
require_text "$ARCH" "PharmaCo360 controlled package generation command release hold ledger"

require_text "$HOLD_LEDGER" "Controlled Package Generation Command Release Hold Ledger"
require_text "$HOLD_LEDGER" "This document does not create a package archive"
require_text "$HOLD_LEDGER" "This document does not execute package generation"
require_text "$HOLD_LEDGER" "This document does not release a package generation command"
require_text "$HOLD_LEDGER" "Required command release hold owners"
require_text "$HOLD_LEDGER" "Command release hold ledger"
require_text "$HOLD_LEDGER" "Required command release evidence"
require_text "$HOLD_LEDGER" "Allowed command release decisions"
require_text "$HOLD_LEDGER" "Command release requirements"
require_text "$HOLD_LEDGER" "Stop conditions"
require_text "$HOLD_LEDGER" "Prohibited action boundary"
require_text "$HOLD_LEDGER" "Default state: command held"

require_text "$HOLD_EXCEPTION_LOG" "Controlled Package Generation Command Release Hold Exception Log"
require_text "$HOLD_EXCEPTION_LOG" "This document does not create a package archive"
require_text "$HOLD_EXCEPTION_LOG" "This document does not execute package generation"
require_text "$HOLD_EXCEPTION_LOG" "This document does not release a package generation command"
require_text "$HOLD_EXCEPTION_LOG" "Command release hold exception log"
require_text "$HOLD_EXCEPTION_LOG" "Required exception types"
require_text "$HOLD_EXCEPTION_LOG" "Exception outcomes"
require_text "$HOLD_EXCEPTION_LOG" "Escalation rules"
require_text "$HOLD_EXCEPTION_LOG" "Default unresolved exception decision: stop"
require_text "$HOLD_EXCEPTION_LOG" "Prohibited action boundary"

echo "== Controlled package generation command release hold ledger source identity =="
...
echo "== Required Phase 16.9 documents =="
...
echo "PharmaCo360 operations controlled package generation command release hold ledger check passed."
