...
for file in "$AUTH_PACKET" "$AUTH_EXCEPTION_LOG" "$HOLD_LEDGER" "$HOLD_EXCEPTION_LOG" "$PREFLIGHT_LEDGER" "$PREFLIGHT_EXCEPTION_LOG" "$RELEASE_LEDGER" "$RELEASE_EXCEPTION_LOG" "$READINESS_LOCK" "$READINESS_UNLOCK_LOG" "$CLOSURE_LEDGER" "$CHECKSUM_LEDGER" "$MANIFEST_LEDGER" "$EXCLUSION_LEDGER" "$EVIDENCE_MANIFEST" "$BINDER" "$AUTH_GATE" "$QA" "$ARCH"; do
  require_file "$file"
done

require_text "$QA" "Phase 17.0 controlled package generation final execution authorization packet"
require_text "$ARCH" "PharmaCo360 controlled package generation final execution authorization packet"

require_text "$AUTH_PACKET" "Controlled Package Generation Final Execution Authorization Packet"
require_text "$AUTH_PACKET" "This document does not create a package archive"
require_text "$AUTH_PACKET" "This document does not execute package generation"
require_text "$AUTH_PACKET" "This document does not release a package generation command"
require_text "$AUTH_PACKET" "Required final execution authorization owners"
require_text "$AUTH_PACKET" "Final execution authorization packet"
require_text "$AUTH_PACKET" "Required final execution authorization evidence"
require_text "$AUTH_PACKET" "Allowed final execution authorization decisions"
require_text "$AUTH_PACKET" "Final execution authorization requirements"
require_text "$AUTH_PACKET" "Stop conditions"
require_text "$AUTH_PACKET" "Prohibited action boundary"
require_text "$AUTH_PACKET" "Default state: not authorized"

require_text "$AUTH_EXCEPTION_LOG" "Controlled Package Generation Final Execution Authorization Exception Log"
require_text "$AUTH_EXCEPTION_LOG" "This document does not create a package archive"
require_text "$AUTH_EXCEPTION_LOG" "This document does not execute package generation"
require_text "$AUTH_EXCEPTION_LOG" "This document does not release a package generation command"
require_text "$AUTH_EXCEPTION_LOG" "Final execution authorization exception log"
require_text "$AUTH_EXCEPTION_LOG" "Required exception types"
require_text "$AUTH_EXCEPTION_LOG" "Exception outcomes"
require_text "$AUTH_EXCEPTION_LOG" "Escalation rules"
require_text "$AUTH_EXCEPTION_LOG" "Default unresolved exception decision: stop"
require_text "$AUTH_EXCEPTION_LOG" "Prohibited action boundary"

echo "== Controlled package generation final execution authorization packet source identity =="
...
echo "== Required Phase 17.0 documents =="
...
echo "PharmaCo360 operations controlled package generation final execution authorization packet check passed."
