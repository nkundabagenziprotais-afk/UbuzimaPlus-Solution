...
section "Phase 20.8 PharmaCo360 Sales/Dispensing Frontend UX Strengthening Check"
...
section "3. Phase 20.7 evidence exists and passed"

PHASE_20_7="docs/pharmaco/sales-dispensing-qa-finding-resolution-plan/PHASE_20_7_PHARMACO360_SALES_DISPENSING_QA_FINDING_RESOLUTION_PLAN_EVIDENCE_OUTPUT.txt"

if [ -f "$PHASE_20_7" ] && grep -q "PHASE 20.7 PHARMACO360 SALES/DISPENSING QA FINDING RESOLUTION PLAN CHECK PASSED" "$PHASE_20_7"; then
  pass "Phase 20.7 evidence exists and passed."
else
  fail "Phase 20.7 evidence missing or failed."
fi

section "4. Phase 20.8 files exist"

DOC_FILE="docs/pharmaco/sales-dispensing-frontend-ux-strengthening/PHASE_20_8_PHARMACO360_SALES_DISPENSING_FRONTEND_UX_STRENGTHENING.md"
STATUS_FILE="docs/pharmaco/sales-dispensing-frontend-ux-strengthening/PHASE_20_8_PHARMACO360_SALES_DISPENSING_FRONTEND_UX_STRENGTHENING_STATUS.env"
COMPONENT_FILE="web/admin-dashboard/src/components/SalesDispensingReview.tsx"
STYLE_FILE="web/admin-dashboard/src/styles.css"

for FILE in "$DOC_FILE" "$STATUS_FILE" "$COMPONENT_FILE" "$STYLE_FILE"; do
  if [ -f "$FILE" ]; then
    pass "$FILE exists."
  else
...
section "5. Phase 20.8 status values are correct"

if [ -f "$STATUS_FILE" ]; then
  if [ "$(read_status_value "$STATUS_FILE" "FRONTEND_UX_STRENGTHENING_STATUS")" = "PREPARED" ]; then
    pass "Frontend UX strengthening status is PREPARED."
  else
    fail "Frontend UX strengthening status is not PREPARED."
  fi
  ...
  if [ "$(read_status_value "$STATUS_FILE" "SOURCE_PLAN_PHASE")" = "PHASE_20_7_QA_FINDING_RESOLUTION_PLAN" ]; then
    pass "Source plan phase is Phase 20.7."
  else
    fail "Source plan phase is incorrect."
  fi

  if [ "$(read_status_value "$STATUS_FILE" "IMPLEMENTATION_STATUS")" = "COMPLETED" ]; then
    pass "Implementation status is COMPLETED."
  else
    fail "Implementation status is not COMPLETED."
  fi
...
section "6. Frontend implementation is included and backend/database remain unchanged"

if [ -f "$STATUS_FILE" ]; then
  if [ "$(read_status_value "$STATUS_FILE" "NEW_IMPLEMENTATION_INCLUDED")" = "YES" ]; then
    pass "New implementation is marked YES."
  else
    fail "New implementation is not YES."
  fi

  if [ "$(read_status_value "$STATUS_FILE" "FRONTEND_CHANGES_INCLUDED")" = "YES" ]; then
    pass "Frontend changes are marked YES."
  else
    fail "Frontend changes are not YES."
  fi

  for KEY in BACKEND_CHANGES_INCLUDED DATABASE_CHANGES_INCLUDED MIGRATION_CHANGES_INCLUDED PRODUCTION_CHANGES_INCLUDED
  do
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "94" ]; then
    pass "Overall system completion is recorded as 94%."
  else
    fail "Overall system completion is not 94%. Current value: ${OVERALL:-MISSING}"
  fi
...
section "9. Sales/dispensing component includes UX strengthening"

if [ -f "$COMPONENT_FILE" ]; then
  if grep -q "paymentStatusTone" "$COMPONENT_FILE" \
    && grep -q "paymentStatusGuidance" "$COMPONENT_FILE" \
    && grep -q "prescriptionGuidance" "$COMPONENT_FILE" \
    && grep -q "requiredPrescriptionCount" "$COMPONENT_FILE" \
    && grep -q "workflow-state-grid" "$COMPONENT_FILE" \
    && grep -q "filter-helper-text" "$COMPONENT_FILE" \
    && grep -q "aria-pressed" "$COMPONENT_FILE" \
    && grep -q "queue-card-active" "$COMPONENT_FILE"; then
    pass "SalesDispensingReview includes Phase 20.8 UX strengthening."
  else
    fail "SalesDispensingReview is missing expected UX strengthening markers."
  fi
fi

section "10. Sales/dispensing styles include responsive strengthening"

if [ -f "$STYLE_FILE" ]; then
  if grep -q "Phase 20.8 — PharmaCo360 sales/dispensing UX strengthening" "$STYLE_FILE" \
    && grep -q ".queue-card-active" "$STYLE_FILE" \
    && grep -q ".filter-helper-text" "$STYLE_FILE" \
    && grep -q ".workflow-state-grid" "$STYLE_FILE" \
    && grep -q ".workflow-state-card" "$STYLE_FILE" \
    && grep -q "@media (max-width: 430px)" "$STYLE_FILE"; then
    pass "Styles include Phase 20.8 responsive UX strengthening."
  else
    fail "Styles are missing expected Phase 20.8 markers."
  fi
fi

section "11. Accidental duplicate sale-status select marker is removed"

if [ -f "$COMPONENT_FILE" ]; then
  if grep -q '<option value="all">All statuses</option>' "$COMPONENT_FILE"; then
    if python3 - "$COMPONENT_FILE" <<'PY_CHECK'
...
raise SystemExit(1 if any(pattern in text for pattern in bad_patterns) else 0)
PY_CHECK
    then
      pass "Duplicate JSX > marker is not present near All statuses."
    else
      fail "Duplicate JSX > marker remains near All statuses."
    fi
  else
    fail "All statuses option not found."
  fi
fi

section "12. Phase 20.8 document describes completed work"

if [ -f "$DOC_FILE" ]; then
  if grep -q "Sales Queue Cards" "$DOC_FILE" \
    && grep -q "Sales Filters" "$DOC_FILE" \
    && grep -q "Selected-Sale Insight Cards" "$DOC_FILE" \
    && grep -q "Payment and Prescription Workflow" "$DOC_FILE" \
    && grep -q "Responsive UX" "$DOC_FILE" \
    && grep -q "Admin dashboard build passed" "$DOC_FILE"; then
    pass "Phase 20.8 document describes completed work."
  else
    fail "Phase 20.8 document is incomplete."
  fi
fi
...
section "14. Only allowed Phase 20.8 files are changed"
...
DISALLOWED_FILES="$(
  echo "$CHANGED_FILES" | grep -Ev \
    '^(web/admin-dashboard/src/components/SalesDispensingReview\.tsx|web/admin-dashboard/src/styles\.css|docs/pharmaco/sales-dispensing-frontend-ux-strengthening/PHASE_20_8_PHARMACO360_SALES_DISPENSING_FRONTEND_UX_STRENGTHENING\.md|docs/pharmaco/sales-dispensing-frontend-ux-strengthening/PHASE_20_8_PHARMACO360_SALES_DISPENSING_FRONTEND_UX_STRENGTHENING_STATUS\.env|docs/pharmaco/sales-dispensing-frontend-ux-strengthening/PHASE_20_8_PHARMACO360_SALES_DISPENSING_FRONTEND_UX_STRENGTHENING_EVIDENCE_OUTPUT\.txt|scripts/pharmaco-sales-dispensing-frontend-ux-strengthening-check\.sh)$' || true
)"

if [ -n "$DISALLOWED_FILES" ]; then
  echo "$DISALLOWED_FILES"
  fail "Files outside the allowed Phase 20.8 frontend UX scope are changed."
else
  pass "Only allowed Phase 20.8 frontend UX files are changed."
fi
...
echo "Overall system completion: approximately 94%"
echo "Phase 20.8 PharmaCo360 sales/dispensing frontend UX strengthening check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 20.8 PHARMACO360 SALES/DISPENSING FRONTEND UX STRENGTHENING CHECK PASSED"
  exit 0
fi

echo "PHASE 20.8 PHARMACO360 SALES/DISPENSING FRONTEND UX STRENGTHENING CHECK FAILED with $FAILURES failure(s)."
exit 1
