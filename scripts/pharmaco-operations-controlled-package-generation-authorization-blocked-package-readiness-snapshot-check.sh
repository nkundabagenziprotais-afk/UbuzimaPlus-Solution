...
section "Phase 19.5 Authorization-Blocked Package Readiness Snapshot Check"
...
PHASE_19_4="docs/pharmaco/controlled-package-generation/final-authorization-non-capture-hold-register/PHASE_19_4_FINAL_AUTHORIZATION_NON_CAPTURE_HOLD_REGISTER_EVIDENCE_OUTPUT.txt"
...
if [ -f "$PHASE_19_4" ] && grep -q "PHASE 19.4 FINAL AUTHORIZATION NON-CAPTURE HOLD REGISTER CHECK PASSED" "$PHASE_19_4"; then pass "Phase 19.4 evidence exists and passed."; else fail "Phase 19.4 evidence missing or failed."; fi

section "4. Phase 19.5 readiness snapshot files exist"

DOC_FILE="docs/pharmaco/controlled-package-generation/authorization-blocked-package-readiness-snapshot/PHASE_19_5_AUTHORIZATION_BLOCKED_PACKAGE_READINESS_SNAPSHOT.md"
STATUS_FILE="docs/pharmaco/controlled-package-generation/authorization-blocked-package-readiness-snapshot/PHASE_19_5_AUTHORIZATION_BLOCKED_PACKAGE_READINESS_SNAPSHOT_STATUS.env"

if [ -f "$DOC_FILE" ]; then
  pass "Phase 19.5 readiness snapshot document exists."
else
  fail "Missing Phase 19.5 readiness snapshot document."
fi

if [ -f "$STATUS_FILE" ]; then
  pass "Phase 19.5 readiness snapshot status file exists."
else
  fail "Missing Phase 19.5 readiness snapshot status file."
fi

section "5. Readiness is confirmed but authorization remains blocked"

if [ -f "$STATUS_FILE" ]; then
  SNAPSHOT_STATUS="$(read_status_value "$STATUS_FILE" "PACKAGE_READINESS_SNAPSHOT_STATUS")"
  ...
  if [ "$SNAPSHOT_STATUS" = "READY_BUT_AUTHORIZATION_BLOCKED" ]; then
    pass "Package readiness snapshot status is READY_BUT_AUTHORIZATION_BLOCKED."
  else
    fail "Package readiness snapshot status is not safe. Current value: ${SNAPSHOT_STATUS:-MISSING}"
  fi
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "89.5" ]; then
    pass "Overall system completion is recorded as 89.5%."
  else
    fail "Overall system completion is not 89.5%. Current value: ${OVERALL:-MISSING}"
  fi
...
section "8. Snapshot documents readiness without authorization"

if [ -f "$DOC_FILE" ]; then
  if grep -q "readiness-reviewed but authorization-blocked" "$DOC_FILE" \
    && grep -q "no valid final owner authorization has been captured" "$DOC_FILE" \
    && grep -q "No production action is approved by this phase" "$DOC_FILE"; then
    pass "Snapshot documents readiness while preserving authorization lock."
  else
    fail "Snapshot missing required readiness/authorization-lock statements."
  fi
fi
...
echo "Overall system completion: approximately 89.5%"
echo "Phase 19.5 authorization-blocked package readiness snapshot check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 19.5 AUTHORIZATION-BLOCKED PACKAGE READINESS SNAPSHOT CHECK PASSED"
  exit 0
fi

echo "PHASE 19.5 AUTHORIZATION-BLOCKED PACKAGE READINESS SNAPSHOT CHECK FAILED with $FAILURES failure(s)."
exit 1
