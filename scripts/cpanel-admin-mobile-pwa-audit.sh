...
require_file "$ADMIN_DIR/public/pwa-safe.html"
...
grep -Fq 'maximum-scale=1' "$ADMIN_DIR/index.html"
...
grep -Fq "ubuzima-admin-shell-v22" "$ADMIN_DIR/public/sw.js"
grep -Fq "ubuzima-admin-failover-v1" "$ADMIN_DIR/public/sw.js"
grep -Fq "refreshShellCache" "$ADMIN_DIR/public/sw.js"
grep -Fq "assetRecoveryResponse" "$ADMIN_DIR/public/sw.js"
grep -Fq "shellRecoveryResponse" "$ADMIN_DIR/public/sw.js"
grep -Fq "promoteFailoverSnapshot" "$ADMIN_DIR/public/sw.js"
grep -Fq "isCacheableAssetResponse" "$ADMIN_DIR/public/sw.js"
grep -Fq "ubuzima-boot-fallback" "$ADMIN_DIR/index.html"
grep -Fq "ubuzimaClearPwaCache" "$ADMIN_DIR/index.html"
grep -Fq "ubuzima-public-site-shell" "$ADMIN_DIR/src/App.tsx"
grep -Fq "readInitialStaffLoginOpen" "$ADMIN_DIR/src/App.tsx"
grep -Fq "profileHasActiveAdminScope" "$ADMIN_DIR/src/App.tsx"
grep -Fq "ubuzima-safe-landing" "$ADMIN_DIR/public/pwa-safe.html"
grep -Fq 'maximum-scale=1' "$ADMIN_DIR/public/pwa-safe.html"
grep -Fq "openStableApp" "$ADMIN_DIR/public/pwa-safe.html"
grep -Fq "registration.update()" "$ADMIN_DIR/src/main.tsx"
grep -Fq "UBUZIMA_ADMIN_APP_READY" "$ADMIN_DIR/src/main.tsx"
grep -Fq "RewriteCond %{REQUEST_URI} ^/admin/assets/" "$ADMIN_DIR/public/.htaccess"
grep -Fq -- "--exclude 'assets/***'" "$ROOT_DIR/scripts/cpanel-admin-mobile-pwa-deploy.sh"
grep -Fq "restore_admin_backup" "$ROOT_DIR/scripts/cpanel-admin-mobile-pwa-deploy.sh"
grep -Fq "verify_live_asset" "$ROOT_DIR/scripts/cpanel-admin-mobile-pwa-deploy.sh"
grep -Fq "verify_live_api_health" "$ROOT_DIR/scripts/cpanel-admin-mobile-pwa-deploy.sh"
...
require_file "$ADMIN_DIR/dist/pwa-safe.html"
