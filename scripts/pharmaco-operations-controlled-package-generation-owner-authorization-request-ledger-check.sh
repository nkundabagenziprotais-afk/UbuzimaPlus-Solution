...
section "Phase 19.0 Owner Authorization Request Ledger Check"
...
section "4. Phase 19.0 owner authorization request remains pending"

STATUS_FILE="docs/pharmaco/controlled-package-generation/owner-authorization-request-ledger/PHASE_19_0_OWNER_AUTHORIZATION_REQUEST_STATUS.env"

if [ -f "$STATUS_FILE" ]; then
  REQUEST_STATUS="$(read_status_value "$STATUS_FILE" "OWNER_AUTHORIZATION_REQUEST_STATUS")"
  OWNER_GRANTED="$(read_status_value "$STATUS_FILE" "OWNER_AUTHORIZATION_GRANTED")"

  if [ "$REQUEST_STATUS" = "PENDING_OWNER_DECISION" ]; then
    pass "Owner authorization request remains pending owner decision."
  else
    fail "Owner authorization request status is not PENDING_OWNER_DECISION. Current value: ${REQUEST_STATUS:-MISSING}"
  fi
  ...
  fail "Missing Phase 19.0 status file: $STATUS_FILE"
fi
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "87" ]; then
    pass "Overall system completion is recorded as 87%."
  else
    fail "Overall system completion is not 87%. Current value: ${OVERALL:-MISSING}"
  fi
...
echo "Overall system completion: approximately 87%"
echo "Phase 19.0 owner authorization request ledger check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 19.0 OWNER AUTHORIZATION REQUEST LEDGER CHECK PASSED"
  exit 0
fi

echo "PHASE 19.0 OWNER AUTHORIZATION REQUEST LEDGER CHECK FAILED with $FAILURES failure(s)."
exit 1
