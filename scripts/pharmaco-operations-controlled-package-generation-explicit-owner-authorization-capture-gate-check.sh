...
section "Phase 19.3 Explicit Owner Authorization Capture Gate Check"
...
section "4. Phase 19.3 capture gate files exist"

DOC_FILE="docs/pharmaco/controlled-package-generation/explicit-owner-authorization-capture-gate/PHASE_19_3_EXPLICIT_OWNER_AUTHORIZATION_CAPTURE_GATE.md"
STATUS_FILE="docs/pharmaco/controlled-package-generation/explicit-owner-authorization-capture-gate/PHASE_19_3_EXPLICIT_OWNER_AUTHORIZATION_CAPTURE_GATE_STATUS.env"

if [ -f "$DOC_FILE" ]; then
  pass "Phase 19.3 capture gate document exists."
else
  fail "Missing Phase 19.3 capture gate document."
fi

if [ -f "$STATUS_FILE" ]; then
  pass "Phase 19.3 capture gate status file exists."
else
  fail "Missing Phase 19.3 capture gate status file."
fi

section "5. Owner authorization remains not captured"

if [ -f "$STATUS_FILE" ]; then
  CAPTURE_STATUS="$(read_status_value "$STATUS_FILE" "OWNER_AUTHORIZATION_CAPTURE_STATUS")"
  ...
  LATEST_INSTRUCTION="$(read_status_value "$STATUS_FILE" "LATEST_OWNER_INSTRUCTION")"
  INSTRUCTION_INTERPRETATION="$(read_status_value "$STATUS_FILE" "LATEST_OWNER_INSTRUCTION_INTERPRETATION")"

  if [ "$CAPTURE_STATUS" = "NOT_CAPTURED" ]; then
    pass "Owner authorization capture status remains NOT_CAPTURED."
  else
    fail "Owner authorization capture status is not NOT_CAPTURED. Current value: ${CAPTURE_STATUS:-MISSING}"
  fi
  ...
  if [ "$LATEST_INSTRUCTION" = "PROCEED" ]; then
    pass "Latest owner instruction is recorded as PROCEED."
  else
    fail "Latest owner instruction is not recorded as PROCEED. Current value: ${LATEST_INSTRUCTION:-MISSING}"
  fi

  if [ "$INSTRUCTION_INTERPRETATION" = "CONTINUE_CONTROLLED_WORKFLOW_ONLY" ]; then
    pass "Latest instruction is interpreted as controlled workflow continuation only."
  else
    fail "Latest instruction interpretation is not safe. Current value: ${INSTRUCTION_INTERPRETATION:-MISSING}"
  fi
fi

section "6. Required owner approval wording exists but is not captured as approval"

if [ -f "$DOC_FILE" ]; then
  if grep -q "I, Protais Nkundabagenzi, explicitly authorize controlled local package generation" "$DOC_FILE" \
    && grep -q "The provided instruction does not match the required explicit authorization wording" "$DOC_FILE"; then
    pass "Required wording is documented and current instruction is marked insufficient."
  else
    fail "Required wording or insufficiency statement is missing."
  fi
fi
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "88.5" ]; then
    pass "Overall system completion is recorded as 88.5%."
  else
    fail "Overall system completion is not 88.5%. Current value: ${OVERALL:-MISSING}"
  fi
...
ENV_VALUES="$(
  printf '%s\n' \
    ...
    "PHARMACO_OWNER_AUTHORIZATION_CAPTURE_STATUS=${PHARMACO_OWNER_AUTHORIZATION_CAPTURE_STATUS:-NOT_CAPTURED}" \
...
echo "Overall system completion: approximately 88.5%"
echo "Phase 19.3 explicit owner authorization capture gate check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 19.3 EXPLICIT OWNER AUTHORIZATION CAPTURE GATE CHECK PASSED"
  exit 0
fi

echo "PHASE 19.3 EXPLICIT OWNER AUTHORIZATION CAPTURE GATE CHECK FAILED with $FAILURES failure(s)."
exit 1
