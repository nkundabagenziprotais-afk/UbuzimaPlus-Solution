...
section "Phase 19.1 Owner Authorization Response Capture Hold Check"
...
if [ -f "$PHASE_18_6" ] && grep -q "PHASE 18.6 DRY-RUN EVIDENCE CHECK PASSED" "$PHASE_18_6"; then
  pass "Phase 18.6 evidence exists and passed."
else
  fail "Phase 18.6 evidence missing or failed."
fi

if [ -f "$PHASE_18_7" ] && grep -q "PHASE 18.7 OWNER AUTHORIZATION GATE CHECK PASSED" "$PHASE_18_7"; then
  pass "Phase 18.7 evidence exists and passed."
else
  fail "Phase 18.7 evidence missing or failed."
fi

if [ -f "$PHASE_18_8" ] && grep -q "PHASE 18.8 FINAL OWNER AUTHORIZATION EVIDENCE PACKET CHECK PASSED" "$PHASE_18_8"; then
  pass "Phase 18.8 evidence exists and passed."
else
  fail "Phase 18.8 evidence missing or failed."
fi

if [ -f "$PHASE_18_9" ] && grep -q "PHASE 18.9 FINAL AUTHORIZATION DECISION HOLD CHECK PASSED" "$PHASE_18_9"; then
  pass "Phase 18.9 evidence exists and passed."
else
  fail "Phase 18.9 evidence missing or failed."
fi

if [ -f "$PHASE_19_0" ] && grep -q "PHASE 19.0 OWNER AUTHORIZATION REQUEST LEDGER CHECK PASSED" "$PHASE_19_0"; then
  pass "Phase 19.0 evidence exists and passed."
else
  fail "Phase 19.0 evidence missing or failed."
fi

section "4. Phase 19.1 owner authorization response remains uncaptured"

STATUS_FILE="docs/pharmaco/controlled-package-generation/owner-authorization-response-capture-hold/PHASE_19_1_OWNER_AUTHORIZATION_RESPONSE_CAPTURE_HOLD_STATUS.env"

if [ -f "$STATUS_FILE" ]; then
  RESPONSE_STATUS="$(read_status_value "$STATUS_FILE" "OWNER_AUTHORIZATION_RESPONSE_STATUS")"
  OWNER_GRANTED="$(read_status_value "$STATUS_FILE" "OWNER_AUTHORIZATION_GRANTED")"

  if [ "$RESPONSE_STATUS" = "NO_EXPLICIT_APPROVAL_CAPTURED" ]; then
    pass "Owner authorization response remains NO_EXPLICIT_APPROVAL_CAPTURED."
  else
    fail "Owner authorization response status is not NO_EXPLICIT_APPROVAL_CAPTURED. Current value: ${RESPONSE_STATUS:-MISSING}"
  fi
  ...
  fail "Missing Phase 19.1 status file: $STATUS_FILE"
fi

section "5. Package generation and production actions remain blocked"
...
section "6. Overall system completion is tracked separately"
...
if [ -f "$STATUS_FILE" ]; then
  ...
  if [ "$OVERALL" = "87.5" ]; then
    pass "Overall system completion is recorded as 87.5%."
  else
    fail "Overall system completion is not 87.5%. Current value: ${OVERALL:-MISSING}"
  fi
...
section "7. Environment authorization overrides are blocked"

ENV_VALUES="$(
  printf '%s\n' \
    "PHARMACO_OWNER_AUTHORIZATION_GRANTED=${PHARMACO_OWNER_AUTHORIZATION_GRANTED:-NO}" \
    "PHARMACO_OWNER_AUTHORIZATION_RESPONSE_STATUS=${PHARMACO_OWNER_AUTHORIZATION_RESPONSE_STATUS:-NO_EXPLICIT_APPROVAL_CAPTURED}" \
...
if echo "$ENV_VALUES" | grep -Eiq '=(GRANTED|AUTHORIZED|YES|TRUE|ALLOW|ALLOWED|GO|APPROVED)$'; then
  fail "Environment-based authorization override detected."
else
  pass "No environment-based authorization override detected."
fi

section "8. No premature package archive/checksum artifacts are tracked"
...
section "9. Protected tracked files remain blocked"
...
echo "Overall system completion: approximately 87.5%"
echo "Phase 19.1 owner authorization response capture hold check: $([ "$FAILURES" -eq 0 ] && echo "100%" || echo "blocked")"
...
if [ "$FAILURES" -eq 0 ]; then
  echo "PHASE 19.1 OWNER AUTHORIZATION RESPONSE CAPTURE HOLD CHECK PASSED"
  exit 0
fi

echo "PHASE 19.1 OWNER AUTHORIZATION RESPONSE CAPTURE HOLD CHECK FAILED with $FAILURES failure(s)."
exit 1
