(() => {
  ...
  const VERSION =
    "2026.08.sales-recording-integrity-v3.8-r1-commercial-determinism";

  if (
    window
      .__UBUZIMA_SALES_RECORDING_INTEGRITY_V3_8_R1__
  ) {
    return;
  }
  ...
  const CACHE_PREFIX =
    "ubuzima.sales.integrity.v3.8.r1.authoritative";
  ...
  function styleRecent(
    ...
    if (viewport) {
      ...
      viewport.style.scrollbarGutter = "auto";
  ...
  function markRecentUnstable() {
    document.documentElement
      .classList
      .remove(
        "ubuzima-v38r1-recent-stable",
      );
  }

  function markRecentStable() {
    document.documentElement
      .classList
      .add(
        "ubuzima-v38r1-recent-stable",
  ...
  function renderRecent(
    ...
    state.recentAuthoritativeRender =
      true;
  ...
  function selectedInsurancePartnerName() {
    const candidates = [];
    ...
    for (
      ...
      if (
        !name
        ||
        /^(select|choose|none|not selected)$/i.test(
          name,
        )
      ) {
        continue;
      ...
      const priority =
        /insurance\s+(partner|provider)/i.test(
          label,
        )
          ? 3
          : /insurance\s+institution/i.test(
              label,
            )
            ? 2
            : 1;
  ...
  function transactionControlValue(
    control,
  ) {
    if (!control) {
      return "";
  ...
  function transactionIdentityMemory() {
    if (
      state.transactionIdentityMemory
      &&
      typeof state.transactionIdentityMemory === "object"
    ) {
      return state.transactionIdentityMemory;
    }
  ...
  function transactionControlDescriptor(
    control,
  ) {
    if (
  ...
  function selectedBusinessOptionName(
    control,
  ) {
    const selected =
      control
  ...
  function rememberTransactionIdentityFromControl(
    control,
  ) {
    if (
      !(control instanceof Element)
    ...
    const descriptor =
      transactionControlDescriptor(
        control,
      );

    const memory =
      transactionIdentityMemory();
    ...
    if (
      ...
      memory.insuranceName =
        selectedBusinessOptionName(
          control,
        )
        ||
        clean(
          selectedInsurancePartnerName(),
        );
  ...
  function clearTransactionIdentityMemory() {
    state.transactionIdentityMemory = {
      customerName: "",
      customerPhoneTin: "",
      insuranceName: "",
...
function transactionIdentitySnapshot() {
    const memory =
      transactionIdentityMemory();
    ...
    const livePhoneTin =
      transactionFieldValue(
        /^customer\s+(?:phone\s*\/\s*tin|phone\s+tin|tin)\b/i,
      );
    ...
    state.lastTransactionIdentitySnapshot =
      {
        ...
        customer_phone_tin_length:
          snapshot
            .customerPhoneTin
            .length,

        insurance_name_present:
          Boolean(
            snapshot.insuranceName,
          ),
      };
...
function enrichAuthoritativeCheckout(
    info,
    input,
    init,
  ) {
    ...
    try {
      ...
      const customerPhoneTin =
        identity.customerPhoneTin
        ||
        clean(
      ...
      if (customerPhoneTin) {
         ...
         * optional exact-nine-digit validation.
         */
        payload.customer_phone_tin =
          customerPhoneTin;
      }
      ...
      if (
        ...
        payload.insurance_partner_name =
          identity.insuranceName;
      }

      state.lastTransactionIdentity =
        {
          customer_name_present:
            Boolean(
              customerName,
      ...
      state.checkoutIdentityEnrichments =
        Number(
          state.checkoutIdentityEnrichments
          || 0,
        )
  ...
  window.fetch =
    async function (
      ...
      if (
        ...
        state.lastCompletedCheckoutIdentity =
          state.lastTransactionIdentitySnapshot
          || null;

        window.setTimeout(
          clearTransactionIdentityMemory,
          0,
        );
      }
...
`;
  ...
  function resetPosCounterScroll() {
    try {
      window.scrollTo(
        0,
        0,
      ...
      Object.freeze({
        ...
        diagnostics() {
          ...
          return {
            ...
            commercial_runtime_layers: 1,

            recent_visible_limit: 10,

            recent_nested_vertical_scroll: false,

            kigali_time_suffix: "NONE",

            insurance_name_last:
              state.lastInsuranceName || "",

            insurance_payload_enrichments:
              Number(state.insurancePayloadEnrichments || 0),

            deterministic_recent_schema: true,

            recent_authoritative_render:
              Boolean(state.recentAuthoritativeRender),

            checkout_identity_enrichments:
              Number(state.checkoutIdentityEnrichments || 0),

            last_transaction_identity:
              state.lastTransactionIdentity || null,

            legacy_r3_cache_reused: false,

            recent_rows_read_only: true,

            recent_row_interaction_policy: "INERT_SUMMARY",

            recent_blocked_interactions:
              Number(state.recentBlockedInteractions || 0),

            identity_capture_mode: "INPUT_CHANGE_MEMORY_PLUS_LIVE_DOM",

            identity_capture_events:
              Number(state.identityCaptureEvents || 0),

            last_identity_snapshot:
              state.lastTransactionIdentitySnapshot || null,

            last_completed_checkout_identity:
              state.lastCompletedCheckoutIdentity || null,
  ...
  console.info(
    "[Ubuzima+] V3.8-R1 commercial deterministic sales loaded.",
    VERSION,
  );

  /*
   * AQUILA V3.8-R1 COMMERCIAL DETERMINISM
   *
   * Recent Sales is deliberately a read-only operational
   * summary. Interacting with a displayed sale row must never
   * alter the table schema, row values, customer identity,
   * filters, sale context or any adjacent information.
   */

  function v38RecentContainer() {
    return document.querySelector(
      ".pos-recent-transactions-bottom",
    );
  }

  function v38RecentRowFromTarget(
    target,
  ) {
    if (
  ...
  function v38BlockRecentPointer(
    event,
    preventDefault,
  ) {
    const row =
      v38RecentRowFromTarget(
        event.target,
      );
    ...
    state.recentBlockedInteractions =
      Number(
        state.recentBlockedInteractions
        || 0,
      )
  ...
  /*
   * Capture transaction identity while the actual Transaction
   * Set-UP control is present.
   */
  document.addEventListener(
    "input",
    event => {
      rememberTransactionIdentityFromControl(
        event.target,
      );
    },
    true,
  ...
  /*
   * Block every meaningful row activation path.
   * Pointer down/up are stopped without preventDefault so page
   * scrolling itself remains natural.
   */
  for (
    const eventName
    of [
