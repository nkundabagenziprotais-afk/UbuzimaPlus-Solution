...
/*
 * AQUILA_POS_MULTIBATCH_FEFO_AVAILABILITY_V2
 *
 * Preserve the current POS markup and CSS. Eligible batches for the
 * same product and branch are represented by one FEFO product entry
 * with their combined available quantity.
 */
function consolidatePosInventoryForFefoCheckout(
  ...
  const ordered = batches
    ...
    .sort((left, right) => {
      const leftExpiry = left.expiry_date
        ? String(left.expiry_date).slice(0, 10)
        : '9999-12-31';

      const rightExpiry = right.expiry_date
        ? String(right.expiry_date).slice(0, 10)
        : '9999-12-31';

      const expiryComparison =
      ...
      const leftReceived = String(
        left.received_at ?? '9999-12-31',
      );

      const rightReceived = String(
        right.received_at ?? '9999-12-31',
      );

      const receivedComparison =
        leftReceived.localeCompare(rightReceived);
  ...
  ordered.forEach((batch) => {
    const extendedBatch =
      batch as PharmaStockBatch & {
        product_id?: number | null;
      };

    const productId = Number(
      batch.product?.id
      ?? extendedBatch.product_id
      ?? 0,
    );

    /*
     * loadCurrentPosInventory is already scoped to the active
     * POS branch. Group only eligible batches of the same product.
     */
  ...
  return [...grouped.values()].map(
    (productBatches) => {
      const primary = productBatches[0];

      const quantityOnHand =
        productBatches.reduce(
          (total, batch) =>
            total
            + Number(batch.quantity_on_hand ?? 0),
          0,
        );
  ...
  function renderPosWorkspace() {
    ...
    async function loadCurrentPosInventory() {
      ...
      try {
        ...
        const batches = consolidatePosInventoryForFefoCheckout(response.batches || []);
