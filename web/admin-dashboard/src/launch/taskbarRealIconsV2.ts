function iconLabel(button: HTMLElement): string {
  return (
...
function decorateButton(button: HTMLElement): void {
  ...
  if (!icon) {
    ...
    icon.className = "ubuzima-source-dock__icon";
    icon.setAttribute("aria-hidden", "true");
    button.prepend(icon);
  }

  icon.innerHTML = svgFor(label);
  icon.setAttribute("data-ubuzima-real-icon-v2", "true");
...
function applyIcons(): void {
  document
    .querySelectorAll<HTMLElement>(".ubuzima-source-dock button, .ubuzima-source-dock__app, .ubuzima-source-dock__task, .ubuzima-mac-dock button")
...
function boot(): void {
  ...
  const schedule = () => {
    ...
    timer = window.setTimeout(applyIcons, 120);
  };
  ...
  window.setInterval(applyIcons, 1500);
}
