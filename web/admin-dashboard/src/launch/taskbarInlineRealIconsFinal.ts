...
function labelOf(node: HTMLElement): string {
  return (
    ...
    node.getAttribute("data-ubuzima-full-label") ||
    node.textContent ||
    "Module"
  ).replace(/\s+/g, " ").trim();
}

function svgIcon(label: string): string {
  ...
  if (/\b(inventory|product inventory|medicine|drug|pill|stock)\b/.test(value)) {
  ...
  if (/\b(report|analytics|statistics)\b/.test(value)) {
  ...
  if (/\b(admin|security|permission|role|access)\b/.test(value)) {
    return `<svg viewBox="0 0 64 64" fill="none"><path d="M32 7 51 15v14c0 13-7 23-19 28C20 52 13 42 13 29V15l19-8Z" fill="#fff" stroke="#0f766e" stroke-width="3"/><rect x="23" y="29" width="18" height="14" rx="4" fill="#ccfbf1" stroke="#0f766e" stroke-width="3"/></svg>`;
  }
...
function normalizeDock(): void {
  const dock = document.getElementById(DOCK_ID) || document.querySelector<HTMLElement>(".ubuzima-source-dock");
  if (!dock) return;

  dock.id = DOCK_ID;
  dock.classList.add("ubuzima-source-dock--inline-real-icons");

  Array.from(dock.querySelectorAll<HTMLElement>(".ubuzima-source-dock__app, .ubuzima-source-dock__task, button, a[role='button']")).forEach((button) => {
    ...
    icon.innerHTML = svgIcon(label);
    icon.setAttribute("data-ubuzima-inline-real-icon", "true");
...
function boot(): void {
  ...
  const schedule = () => {
    ...
    timer = window.setTimeout(normalizeDock, 100);
  };
  ...
  window.setInterval(normalizeDock, 1000);
}
