...
function iconForLabel(label: string): string {
  ...
  if (/\b(pos|counter|cashier|till|checkout)\b/.test(value)) return "./dock-icons/pos.svg";
  if (/\b(product master|product setup|medicine|drug|catalog)\b/.test(value)) return "./dock-icons/product-master.svg";
  if (/\b(inventory|product inventory|stock quantity|available stock)\b/.test(value)) return "./dock-icons/inventory.svg";
  if (/\b(general stock|warehouse|store stock|stock room)\b/.test(value)) return "./dock-icons/general-stock.svg";
  ...
  if (/\b(insurance|claim|claims|policy)\b/.test(value)) return "./dock-icons/insurance.svg";
  if (/\b(supplier|vendor|delivery)\b/.test(value)) return "./dock-icons/suppliers.svg";
  if (/\b(finance|payment|cash|bank|money|account)\b/.test(value)) return "./dock-icons/finance.svg";
  if (/\b(report|analytics|dashboard report|statistics)\b/.test(value)) return "./dock-icons/reports.svg";
  if (/\b(user|staff|employee|pharmacist|doctor|patient)\b/.test(value)) return "./dock-icons/users.svg";
  if (/\b(admin|security|permission|role|access)\b/.test(value)) return "./dock-icons/admin.svg";
  if (/\b(setting|configuration|preference)\b/.test(value)) return "./dock-icons/settings.svg";
  if (/\b(pharmacy|clinic|dispensary)\b/.test(value)) return "./dock-icons/pharmacy.svg";
