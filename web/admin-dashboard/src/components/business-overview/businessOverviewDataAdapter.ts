...
export async function loadBusinessOverviewDataAdapter({
  ...
  return {
    ...
    inventoryRows: [
      ...
      { label: 'Stock Batches', value: inventoryLoaded ? `${formatCount(inventory!.stockBatches)} batches · ${formatMoney(inventory!.stockBatchValue)}` : '—' },
      { label: 'Healthy Stock', value: inventoryLoaded ? `${formatCount(inventory!.healthy)} batches · ${formatMoney(inventory!.healthyValue)}` : '—' },
      { label: 'Low Stock Items', value: inventoryLoaded ? `${formatCount(inventory!.lowStock)} batches · ${formatMoney(inventory!.lowStockValue)}` : '—' },
      { label: 'Expiring Items', value: inventoryLoaded ? `${formatCount(inventory!.expiring)} batches · ${formatMoney(inventory!.expiringValue)}` : '—' },
      { label: 'Expired Batches', value: inventoryLoaded ? `${formatCount(inventory!.expired)} batches · ${formatMoney(inventory!.expiredValue)}` : '—' },
    ],
    paymentMix,
    topProducts: [],
    trend: sales.trend,
