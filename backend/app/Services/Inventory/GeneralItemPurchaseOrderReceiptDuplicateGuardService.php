...
final class GeneralItemPurchaseOrderReceiptDuplicateGuardService
{
    private const OPERATION_TYPE =
        'general_item_purchase_order_receipt';
    ...
    public function begin(
        ...
        /*
         * Compatibility mode is temporary while the frontend
         * confirmation workflow is being introduced.
         */
        if ($idempotencyKey === '') {
            return null;
        }
        ...
        if (! $line || ! $line->generalItem) {
            throw ValidationException::withMessages([
                'pharmaco_general_purchase_order_item_id' => [
                    'Select a valid General Item Purchase Order line.',
                ],
            ]);
        }
        ...
        if (! $location) {
            throw ValidationException::withMessages([
                'pharmaco_general_item_location_id' => [
                    'Select an active General Item stock location.',
                ],
            ]);
        }
        ...
        $reservation =
            $this->detector->reserveGuard([
                ...
                'source_line_type' =>
                    'pharmaco_general_purchase_order_item',
        ...
        if ($reservation['payload_mismatch']) {
            $this->throwJson([
                'message' =>
                    ...
                    . 'for different Purchase Order receipt information.',
        ...
        if (
            ...
            if ($movement) {
                $this->throwJson([
                    'message' =>
                        'This Purchase Order receipt was already recorded.',
        ...
        if (
            ...
            $this->throwJson([
                'message' =>
                    'This Purchase Order receipt is already being processed.',
    ...
    public function revalidateOrFail(
        ...
        Tenant $tenant,
        PharmacoPurchaseOrder $purchaseOrder,
        PharmacoGeneralPurchaseOrderItem $line,
        PharmacoGeneralItemLocation $location,
        array $validated,
        ?User $user
    ): void {
        $item = $line->generalItem()
            ->where('tenant_id', $tenant->id)
            ->firstOrFail();
    ...
    private function evaluateOrFail(
        ...
        if (! $overrideAllowed) {
            $this->throwJson([
                'message' =>
                    ...
                    . 'a suspected duplicate Purchase Order receipt.',
                'code' =>
        ...
        if ($reason === '') {
            $this->throwJson([
                ...
                'errors' => [
                    'duplicate_override_reason' => [
                        'Explain why this Purchase Order receipt is legitimate.',
                    ],
                ],
            ], 422);
        }
        ...
        if (
            ...
            $this->throwJson([
                'message' =>
                    'The duplicate confirmation token is invalid or expired.',
                'code' =>
