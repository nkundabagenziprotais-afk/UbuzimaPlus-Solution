...
/* AQUILA_FINANCE_R50C_R7B_LIVE_POS_POSTING */
...
class PharmacoPosPaymentLivePostingService
{
    ...
    public function postPayment(PharmacoPayment $payment, PharmacoSale $sale): FinanceJournalEntry|FinancePostingLog
    {
        ...
        $lines = [
            new FinanceJournalLinePayload(
                ...
                debit: $amount,
                description: "POS payment {$payment->receipt_number}",
                lineType: 'payment',
        ...
        if ($taxCredit > 0) {
            $lines[] = new FinanceJournalLinePayload(
                ...
                credit: $taxCredit,
                description: "POS tax for payment {$payment->receipt_number}",
                lineType: 'tax',
                branchId: $sale->branch_id,
                customerId: $sale->pharmaco_customer_id,
                paymentMethod: $payment->payment_method,
        ...
        if ($revenueCredit > 0) {
            $lines[] = new FinanceJournalLinePayload(
                ...
                description: "POS revenue for payment {$payment->receipt_number}",
                lineType: 'revenue',
                branchId: $sale->branch_id,
                customerId: $sale->pharmaco_customer_id,
                paymentMethod: $payment->payment_method,
        ...
        return $this->postingService->post(new FinancePostingPayload(
            ...
            idempotencyKey: "pos-payment-live-{$payment->id}",
            lines: $lines,
            currencyCode: 'RWF',
            memo: "POS payment {$payment->receipt_number} for sale {$sale->sale_number}",
