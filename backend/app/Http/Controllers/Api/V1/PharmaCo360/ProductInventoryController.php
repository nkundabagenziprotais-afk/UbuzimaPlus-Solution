...
class ProductInventoryController extends Controller
{
    ...
    public function analyticsSummary(\Illuminate\Http\Request $request)
    {
        ...
        $totalInventoryValue = (clone $batchBase)
            ->sum(\Illuminate\Support\Facades\DB::raw($batchValueExpression));

        $stockOnHandCount = (clone $batchBase)
            ->sum(\Illuminate\Support\Facades\DB::raw('COALESCE(quantity_on_hand, 0)'));

        $lowStock = (clone $batchBase)
            ->whereRaw('COALESCE(quantity_on_hand, 0) > 0')
            ->whereRaw('COALESCE(quantity_on_hand, 0) <= 5');

        $lowStockValue = (clone $lowStock)
            ->sum(\Illuminate\Support\Facades\DB::raw($batchValueExpression));
        ...
        $nearExpiryValue = (clone $nearExpiry)
            ->sum(\Illuminate\Support\Facades\DB::raw($batchValueExpression));
